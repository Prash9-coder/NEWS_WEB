...
const Epaper = () => {
    ...
    // States
    const [selectedDate, setSelectedDate] = useState(getAvailableDates()[0].value);
    ...
    useEffect(() => {
        document.title = selectedEdition
            ? `${selectedEdition.name} - E-Paper | NEWS HUB`
            : 'E-Paper | NEWS HUB';
    }, [selectedEdition]);
    ...
    useEffect(() => {
        if (editionParam) {
            const edition = findEditionById(editionParam);
            if (edition) {
    ...
    }, [editionParam, urlPage]);


    // In Epaper.jsx, update the loadPages function

    const loadPages = (edition) => {
        setIsLoading(true);
        setImageLoading(true);

        setTimeout(() => {
            // This will now check localStorage first, then fallback to placeholders
            const newPages = generatePagesForEdition(edition.id, edition.pagesCount, selectedDate);
            setPages(newPages);
            setIsLoading(false);
            setZoomLevel(1);
        }, 300);
    ...
    const handleDownload = () => {
        if (pages[currentPage - 1]) {
            ...
            link.download = `${selectedEdition?.name}-page-${currentPage}.jpg`;
            link.click();
        }
    };

    const handlePrint = () => {
        const printWindow = window.open(pages[currentPage - 1]?.hdImage, '_blank');
        printWindow?.print();
    };
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            await navigator.share({ title: `${selectedEdition?.name} E-Paper`, url });
        } else {
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-xl font-bold text-gray-700">E-Paper</span>
                        </div>
                                    ...
                                    <>
                                        ...
                                        <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border p-4 z-50 w-80">
                                            <div className="flex items-center justify-between mb-3 pb-2 border-b">
                                                <h4 className="font-bold text-gray-800">Select Date</h4>
                    ...
                    .map((category) => (
                        <div key={category.id} className="mb-8">
                            ...
                            >
                                ...
                                <span className="text-sm text-gray-500">({category.editions.length} editions)</span>
                            </div>
                                    ...
                                    >
                                        <div className="relative aspect-[3/4] overflow-hidden bg-gray-100">
                                            ...
                                            <div className="absolute bottom-0 left-0 right-0 p-3 text-white">
                                                <h3 className="font-bold text-base leading-tight">{edition.name}</h3>
                                                <p className="text-xs text-white/80">{edition.nameLocal}</p>
                                            </div>
            ...
            <footer className="bg-secondary text-white py-6 mt-8">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <p className="text-white/60 text-sm">
                        © {new Date().getFullYear()} NEWS HUB E-Paper. All rights reserved.
                    </p>
                </div>
            </footer>
        </div>
            ...
            <header className="bg-[#111] text-white flex-shrink-0 border-b border-white/5 h-10">
                <div className="flex items-center justify-between h-full px-2">
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            onClick={handleBackToEditions}
                            className="p-1.5 hover:bg-white/10 rounded transition-colors"
                        >
                            <FiChevronLeft size={18} />
                        </button>

                        <a href="/" className="hidden md:flex items-center text-sm font-black">
                            <span className="text-primary">NEWS</span>
                    ...
                    <div className="flex items-center gap-0.5">
                        ...
                        <button onClick={handleDownload} className="p-1 hover:bg-white/10 rounded">
                            <FiDownload size={12} />
                        </button>
                        <button onClick={handleShare} className="p-1 hover:bg-white/10 rounded">
                            <FiShare2 size={12} />
                        </button>
                    ...
                    {isLoading ? (
                        ...
                        <>
                            ...
                            >
                                ...
                                {/* FULL WIDTH NEWSPAPER - Scrollable vertically */}
                            ...
                            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black/80 text-white px-4 py-1.5 rounded-full text-xs z-20 backdrop-blur-sm">
                                <span className="font-semibold">{selectedEdition?.name}</span>
                            ...
                            <div className="absolute bottom-2 right-2 text-white/30 text-[9px] hidden lg:block">
                                ← → Pages • ↑ ↓ Scroll • +/- Zoom • Space Scroll Down
                            </div>
                        </>
                    )}
                </div>
